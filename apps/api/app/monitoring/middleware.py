...
def render_metrics() -> str:
    ...
    with _metrics_lock:
        for (method, path, status), value in sorted(_request_counts.items()):
            lines.append(f'fcs_requests_total{{method="{method}",path="{path}",status="{status}"}} {value}')

        lines.append("# HELP fcs_request_errors_total HTTP requests that resulted in errors")
        lines.append("# TYPE fcs_request_errors_total counter")
        if _error_counts:
            for (method, path, status), value in sorted(_error_counts.items()):
                lines.append(f'fcs_request_errors_total{{method="{method}",path="{path}",status="{status}"}} {value}')
        else:
            lines.append('fcs_request_errors_total{method="",path="",status=""} 0')

        lines.append("# HELP fcs_request_duration_seconds_sum Total time spent handling requests")
        ...
        lines.append("# HELP fcs_request_duration_seconds_count Total number of timed requests")
        lines.append("# TYPE fcs_request_duration_seconds_count counter")
        for (method, path), stats in sorted(_latency_stats.items()):
            lines.append(f'fcs_request_duration_seconds_count{{method="{method}",path="{path}"}} {stats.count}')
